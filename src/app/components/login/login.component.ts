...
export class LoginComponent implements OnInit {
  ...
  constructor(private crudMethods: CrudMethodsService,
    private dataService: DataServiceService,
    private shared: SharedMethodsService, private router: Router) { }

  ngOnInit(): void {
  }

  /**
   * Catches the loginCredentials event from Form components and gets the login credentials and routes to the appropriate feature, returns void
  ...
  loginToFeature(email: string, id: string, tokenId: string): void {
    if (email.includes('@student')) {
      this.crudMethods.getMethod(firebase_urls.studentInfo)
        .subscribe((res) => {
          let stu_details = {};
          for (let key in res) {
      ...
      this.shared.openSnackBar('Other users will be enabled shortly', 'Okay', 2000);
